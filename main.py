...
def admin_panel(message):
    ...
    user_id = message.from_user.id
    print(f"[DEBUG] Получена команда /admin от пользователя {user_id}")
    
    # Проверка прав администратора
    if user_id not in config.ADMINS:
        print(f"[DEBUG] Пользователь {user_id} НЕ в списке ADMINS. Выход.")
        bot.send_message(message.chat.id, "❌ У вас нет прав администратора.")
        return
    
    print(f"[DEBUG] Пользователь {user_id} является админом. Показываю панель.")
...
def handle_callbacks(call):
    print(f"[DEBUG] Обработка callback: {call.data}")
...
def handle_admin_callback(call):
    try:
        ...
        print(f"[DEBUG] ERROR in handle_admin_callback: {e}")
...
if __name__ == '__main__':
    ...
    # Принудительно удаляем вебхук и ждём
    print("🔄 Принудительно удаляю вебхук и жду...")
    try:
        import requests
        requests.get(f"https://api.telegram.org/bot{config.BOT_TOKEN}/deleteWebhook")
        print("✅ Вебхук удалён через API")
    except Exception as e:
        print(f"⚠️  Не удалось удалить вебхук через API: {e}")
    
    # Удаляем вебхук с обработкой ошибок
    try:
        bot.remove_webhook()
        print("✅ Вебхук удалён успешно")
    except Exception as e:
        print(f"⚠️ Игнорируем ошибку удаления вебхука: {e}")
        print("🚀 Продолжаем запуск бота в polling режиме...")
    ...
    try:
        ...
        print("🔄 Пробую перезапустить через 10 секунд...")
        time.sleep(10)
